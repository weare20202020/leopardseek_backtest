...
class INFLECTION(Strategy):
    ...
    def __init__(
        self, parameters: dict, period: str, instrument: str, broker: Broker, *args, **kwargs
    ) -> None:
    ...
    def generate_signal(self, dt: datetime):
        ...
        # Get OHLCV data
        count = 105
        data = self.broker.get_candles_backtest(self.instrument, granularity=self.period, count=count, end_time=dt) #最新的时间在最后一行
        data['middle'] = (data['Open'] + data['Close'])/2
        if len(data) < 105:
        ...
        if self.position == 0:
            if self.signal == 1 and comfirm == 1:
                ...
                # No signal
                order = None
        elif self.position == 1 and (comfirm == -1 or self.signal ==-1):
            order = Order(
                instrument=self.instrument,
                direction=-1,
                size=10
